# CLAUDE.md â€” GoldKiosk Platform (Enterprise Build)
...
flow coordinators, SQL, and test scenarios from it â€” never copy code that violates the
standards docs.

## 2. Solution structure & naming (canonical â€” do not invent alternatives)
...
src/
  ...
  kiosk/
    ...
    GoldKiosk.Kiosk.Devices/           # hardware SDK adapters (dispenser, scale, printer, cameraâ€¦)
...
tests/                                 # NUnit only â€” see testing standards
...
- **Frontend (kiosk) = Blazor Hybrid** â€” BlazorWebView in a WPF shell. Hardware lives behind `GoldKiosk.Kiosk.Api` (REST + SignalR), never in-process with the UI.
- **Two APIs**: `Kiosk.Api` on the machine owns hardware + local durability; `Cloud.Api` is the shared backend. All edgeâ†’cloud writes are idempotent via the outbox.
- **Edge decisions locked 2026-07-02** (see `docs/adr/`): no kiosk-local SQL â€” durability is file-based transaction folders in the legacy layout (ADR 0002); kiosks talk exclusively to `Cloud.Api`, never CRM (ADR 0003); every device selects mock/real per device via layered config, cloud overrides local (ADR 0004); DB baseline = platform2 schema sets with enumerated fixes (ADR 0005).
...
- **Configuration**: per-machine `kiosk-settings.json` (ProgramData) + cloud layers â€” Key Vault for secrets, Blob for images/static JSON config with local cache. See configuration-and-operations standard.
...
2. **Never run `dotnet test`** â€” the user runs tests in Visual Studio. Verify with `dotnet build GoldKiosk.slnx -warnaserror` (must pass) and `dotnet format --verify-no-changes`.
...
4. **Postgres conventions are sacred** (Â§3). Never altered as a side effect.
5. **Additive discipline.** No drive-by refactors, no restructuring outside ticket scope.
6. **Fixed dev launch ports** (see plan Â§Ports) so Postman/collections keep working.
...
- [ ] NUnit tests written by **test-writer** for new/changed behavior (â‰¥ 80% branch on new code; money/settlement paths 100%).
...
1. **architect** â€” design note first for anything spanning >1 project, touching Contracts, data model, MSIX layout, or config layering. Binding once approved.
2. **developer** â€” implements per note + standards.
3. **test-writer** â€” NUnit coverage; never edits production code.
4. **code-reviewer** â€” read-only diff review; fixed verdict format.
