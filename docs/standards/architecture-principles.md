...
Dependency rule: **hosts â†’ Application â†’ Domain**; Infrastructure implements Application
...
- **GoldKiosk.Contracts**: versioned wire DTOs + ProblemDetails type codes + SignalR contracts shared edgeâ†”cloud. References nothing; additive changes only within a version.
...
- **Kiosk.Core**: edge-local domain â€” durable transaction queue/outbox, price-staleness policy, session state machine. No cloud types.
...
- **I**: narrow ports â€” `IGoldPriceProvider.GetSpotPriceAsync`, not `IExternalServices`.
...
- **Local durability first**: every customer transaction is persisted by Kiosk.Api locally (file-based transaction folders per ADR 0002 â€” no kiosk-local SQL) before any cloud call; a **queue-and-forward outbox** syncs to Cloud.Api.
- **Idempotency keys** on all edgeâ†’cloud writes; cloud endpoints are idempotent consumers; reconciliation compares edge outbox vs cloud ledger and alerts on discrepancy â€” never auto-fixes silently.
- **Gold price staleness policy** in Kiosk.Core: cached price with explicit TTL; stale price locks buy/sell with a clear customer-facing state â€” never a silent wrong price.
...
## 6. Data
- One PostgreSQL 18 cluster; separate databases: platform (`goldkiosk`) and CRM (`goldkiosk_crm`). No cross-database joins â€” integrate via Application layer or events.
...
- Dashboards: OIDC; kiosk machines: per-device identity (cert/credential) â€” device id flows into every audit record and telemetry item.
- Policy-based, resource-scoped authorization (kiosk operator sees only their kiosk; CRM roles â‰  Admin roles).
